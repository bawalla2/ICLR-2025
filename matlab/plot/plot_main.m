...
for i = 1:numgroups
    ...
    % Call plot function
    [alg_settingsi, out_datai, group_settingsi] = plot_preset_group(...
        alg_settingsi, out_datai, group_settingsi, master_settings);

    % Update out_data and group_settings for the current group
    alg_settings_cell_master{i} = alg_settingsi;
    out_data_cell_master{i} = out_datai;
    group_settings_master{i} = group_settingsi;
    figcount = group_settingsi.figcount;

    % Check for if is an IC or modeling error sweep
    % Save data if applicable
    if (group_settingsi.issweep_IC || group_settingsi.issweep_nu ... 
            || group_settingsi.issweep_rand_nu) ...
    if savedata

        % Make directory to save data to
        if group_settingsi.istraining
            relpath_data = [master_settings.relpath_dirl_sweep];
        else
            relpath_data = [master_settings.relpath_dirl_sweep_eval ...
                group_settingsi.refcmd '/'];
        end

        mkdir(relpath_data)
  

        % Save data -- alg_settings_cell_master struct
        varname = 'alg_settings_cell_master';
        save([relpath_data varname], varname);
        
        % Save data -- out_data_cell_master struct
        varname = 'out_data_cell_master';
        save([relpath_data varname], varname);
        
        % Save data -- group_settings_master struct
